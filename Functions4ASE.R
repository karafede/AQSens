...
# 190110 MG   Etalonnage: solve error when all s_y are NA
#           
# TO BE DONE:
...
Plot_Dist_Ref_log <- function(counts_Ref, Model.i = NULL) {
...
Distribution_OPC_Sensor <- function(General.df, DateBegin= NULL, DateEnd=NULL, bins_diameters, Sensor_dates = NULL, Dist_Ref.full = NULL) {
    ...
    # bins_OPC <- colMeans(bins_OPC, na.rm = T)
    # bins_OPC <- as.data.frame((bins_OPC))
    # bins_OPC$diameters <- rownames(bins_OPC)
    # rownames(bins_OPC) <- NULL
    # names(bins_OPC) <- c("counts", "diameters")
    ...
    Met_OPC <- Met_OPC %>%
        dplyr::summarise(OPCTemp = mean(OPCTemp, na.rm = T),
                         OPCHum = mean(OPCHum, na.rm = T),
                         OPCVol = mean(OPCVol, na.rm = T),
                         OPCTsam = mean(OPCTsam, na.rm = T))
    
    return(list(bins_OPC = bins_OPC, counts_OPC = counts_OPC, Met_OPC = Met_OPC))
...
Plot_Distribution_OPC_Sensor <- function(counts_OPCN, predict_OPC) {
    
    
    plot <-  ggplot() + 
        theme_bw() +
        geom_point(data = counts_OPCN, aes(log10(diameters), log10(counts), col = "OPC"), stat="identity") +
        geom_point(data = predict_OPC, aes(diameters, predict_counts, col = "Ref"), stat="identity") +
