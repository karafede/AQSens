...
#================================================================CR
# New release v0.12
# 2019-01-15 : N71 - The MainTabPanel "MultiLinear" (before Multivariates) now shows the content of the MultiLinear file of the selected sensor. The display content of the MultiLinear is updated every time a file is saved.
#              E27 - Bug Correction in Ind.Sens.Out: Outliers.Sens$Forced was never set to TRUE when the columns c("Out.", "Out.Warm.", "Out.TRh.", "Out.Invalid.", "Out.Warm.TRh.", "Out.Warm.TRh.Inv." were missing from DF$General while the file
#                    Ind.Sens.Out.Rds existed. This is now solved see # Reactive ind.sens.out. The same bug was corrected for Ind.Ref.Out
#              E28 - Bug Correction in plot calibration after deleting a Multilinear calibration model, if the next selected calibration is automatically set to model without covariates an error occurs as Covariates.Model is character(=) 
#                    and cannot be tested with if. This bug is solved by adding a test (if (identical(Covariates.Model, character(0))))  in plot.calibration() 
...
#              E31 - Bug Correction for selecting Valid dates when new reference or sensor data are uploaded. It was not possible to select the dates of new data in NavBarmenu "DataTreatment", sideBarLayout SetTime. Corrected.
#              E32 - Bug Correction: everytime that the App was launched, the file of reference data was saved even if no new reference data were added, wasting cpu time. Corrected: now the file is saved only if new data are added.
...
# 2019-02-08   E35 - in nNavbarMenuu GetData" - "Reference Data" when using option "csv", the csv or Rdata file adds now new parameter for prvious date, before it was only adding values for new dates
# 2019-01-31:  N84 - Added interactive map in the covariates, calibration and extrapolation times series
...
# 2019-02-14:  N86 - Adding new model for calibration of NOB4 sensor: NOB4_CexpkT and NOB4_CexpkT Ri  in nA or V = a0 + a1 NO + C exp(k Temperature) where tem?erature is in ?C or kelvin, respectively.
#              E36 - Bug in the detection of outliers Out.my.Outliers (Fucntions4ASE.R), when the minimum of the interval of tolerance zmin was < Thresholdmin, zmin was not correctly set to ThresholdMin, with a mistake for zmax in plotting.
...
#              E39 - MainTabPanel "Calibration" and "Prediction" in the residuals matric, the coefficients of correlation (r) were displayed instead of the Coefficients of determination (R2) 
#                    ReadRDS change to rdata
#                    calibration one sensor at ta time
