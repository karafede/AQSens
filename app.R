        ...
        if (input$Selected > 0) {
            ...
            output$uicoord.ref      <- renderUI({
                textInput(inputId = "coord.ref", 
                          label   = "Longitude and latitude of the reference station, blk separated", 
    ...
    observeEvent(input$Merge,{
        ...
        output$ts_RawData_dygraphs <- renderUI({
            ...
            if (nrow(General.df) > 1) {
                ...
                time_series_RawData <- data_frame_to_timeseries(General.df)
        ...
        StatFiltered               <- reactive({
            ...
            StatFiltered   <- data.frame(
                ...
                Negative.Reference  = sapply(seq_along(list.gas.reference2use()), function(i) if (Outliers_Ref()$remove.neg[i.sensors()[i]]) length(ind.neg[[i]]) else NA),
                Refe.Outlier_Max    = sapply(seq_along(list.gas.reference2use()), function(i) if (Outliers_Ref()$Ref.rm.Out[i.sensors()[i]]) length(unlist(which(ind.ref$out[paste0(list.gas.reference2use()[i],".1.Outli")][[1]][,"OutliersMax"]))) else NA),
                Refe.Outlier_Min    = sapply(seq_along(list.gas.reference2use()), function(i) if (Outliers_Ref()$Ref.rm.Out[i.sensors()[i]]) length(unlist(which(ind.ref$out[paste0(list.gas.reference2use()[i],".1.Outli")][[1]][,"OutliersMin"]))) else NA),
                Refe.High_values    = sapply(seq_along(list.gas.reference2use()), function(i) if (Outliers_Ref()$Ref.rm.Out[i.sensors()[i]]) length(unlist(which(ind.ref$out[paste0(list.gas.reference2use()[i],".1.Outli")][[1]][,"High_values"]))) else NA),
                Refe.Low_values     = sapply(seq_along(list.gas.reference2use()), function(i) if (Outliers_Ref()$Ref.rm.Out[i.sensors()[i]]) length(unlist(which(ind.ref$out[paste0(list.gas.reference2use()[i],".1.Outli")][[1]][,"Low_values"]) )) else NA)
            )
        ...
        output$ts_Cov_dygraphs <- renderUI({
            ...
            if (all(is.na(DF$General[,INFLUX()[[4]]]))) {
                ...
                cat(paste0("[shiny, ts_Cov_dygraphs()] INFO,Plot sensor data in volt with covariates for Sensor ", 
                ...
                mydata = subset(DF$General[,c("date",Relationships)], date >= input[[paste0("Date",CalSet()$k)]][1] & date <= input[[paste0("Date",CalSet()$k)]][2])
                time_series_sensor_Cov <- data_frame_to_timeseries(mydata)
                # colour_vector <- threadr::ggplot2_colours(45)
                                    ...
                                    if (file.exists(name.Model.i)) { # & input[[paste0("Cal",k)]] != ""
                                        ...
                                        if (grepl(pattern = "rdata", x = name.Model.i)) {
                                            ...
                                            if (file.exists(name.Model.i)) {
                                                ...
                                                # Current model
                                                NewModel  <-  sub(pattern = ".rds", replacement = ".rdata", x = name.Model.i)
        ...
        CalSet     <- eventReactive({
            ...
            if (is.null(NewCalSet)) NewCalSet <- Models[1]
        ...
        pointsCal <- reactive( {
            ...
            PointsCal <- DF$General[DF$General$date >= DateIN & DF$General$date <= DateEND, 
                                    grep(pattern = paste0(c("latitude","longitude", "Ref.Long", "Ref.Lat"), collapse = "|" ), x = names(DF$General))]
            
            # coordinates of the REFERENCE Station
            Ref.coord <- CalSet()$Coord.Ref
            ...
            } else if (exists("Ref.coord") && !is.null(Ref.coord) && grepl(pattern = paste0(c(" ", ","), collapse = "|"), x = Ref.coord)) { 
                
                # checking if the separator is ,
                if (any(grepl(pattern =  ",", x = Ref.coord))) {
                    
                    # Checking is the coordinates are in spherical or decimal format, projection to OpenStreet map
                    if (grep(pattern = paste0(c("N","S", "E", "W", "d"), collapse = "|" ), x = names(DF$General))) {
                        
                        # Here we still have to project the spherical coordinates in Mercator web WS84 of OPenStreet view ####################################################################
                        Ref.coord_LON <- as.numeric(unlist(strsplit(x = Ref.coord, split = ","))[1])
                        Ref.coord_LAT <- as.numeric(unlist(strsplit(x = Ref.coord, split = ","))[2])
                    } else {
                        Ref.coord_LON <- as.numeric(unlist(strsplit(x = Ref.coord, split = ","))[1])
                        Ref.coord_LAT <- as.numeric(unlist(strsplit(x = Ref.coord, split = ","))[2])
                    }
       ...
       # })
                    ...
                    if (NewModelFlag) {
                        ...
                        if (all(is.na(General[,CalSet()$nameGasRef])) | all(is.na(General[,CalSet()$nameGasVolt]))) {
                            ...
                            NewModel  <- paste0(paste(CalSet()$Sens.raw.unit, CalSet()$mod.eta.model.type,
                                                      format(DateIN,"%Y%m%d"),format(DateEND,"%Y%m%d"), namesCovariates, sep = "__"),".rdata")
                            updateSelectInput(session, inputId = paste0("Cal", CalSet()$k), label = "Select a previous calibration ", 
                                              choices = Newchoices, selected = NewModel[1])
                        }
                    } else { 
                    ...
                    if (grepl(pattern = glob2rx(paste0("*_",CalSet()$Sens.raw.unit,"*_",CalSet()$mod.eta.model.type,"_*")), x = CalSet()$Cal)) { 
                        ...
                        if (!(CalSet()$mod.eta.model.type == "MultiLinear") | 
                            ...
                            if (grepl(pattern = glob2rx(paste0("*_",
                                ...
                                if (!(CalSet()$NewCalSet %in% c("ExpGrowth", "exp_kT", "exp_kK", "T_power", "K_power", "MultiLinear"))) Cal_Line(x             = x, 
                                                                                                                      s_x           = NULL, 
                                                                                                                      y             = y, 
                                                                                                                      s_y           = NULL, 
        ...
        Table.SummaryCal     <- reactive({
            ...
            return(list(Model.i$Call,Model.i$Tidy, Model.i$Glance))
        ...
        output$ts_Cal_dygraphs <- renderDygraph({
            ...
            # progress <- shiny::Progress$new()
            # progress$set(message = "[shiny, ts_cal_dygraphs()] INFO, plotting time series of Calibration data", value = 0.5)
            # # Make sure it closes when we exit this reactive, even if there's an error
            # on.exit(progress$close())
            ...
            cat("\n")
            cat("-----------------------------------------------------------------------------------\n")
            cat("[shiny, ts_cal_dygraphs()] INFO, plotting time series of Calibration data\n")
            ...
            if (all(is.na(DF$General[,c(CalSet()$nameGasRef,CalSet()$nameGasVolt)]),na.rm = TRUE)) {
                ...
                if (nrow(Pattern) > 0) {
                    Name.pol <- c(gsub(pattern = Pattern[1,1], replacement = Pattern[1,2], x = CalSet()$nameGasRef), 
                                  gsub(pattern = Pattern[1,1], replacement = Pattern[1,2], x = CalSet()$nameGasMod))
                } 
                if (nrow(Pattern) > 1) {
                ...
                mydata = subset(DF$General, date >= DateIN & date <= DateEND)
                mydata <- mydata %>%
                    dplyr::select(date,
                           CalSet()$nameGasRef,
                           CalSet()$nameGasMod)
                time_series_sensor <- data_frame_to_timeseries(mydata)
                ...
                plot_cal <- dygraph(ts_cal, 
                    ...
                    dySeries(CalSet()$nameGasRef,label = CalSet()$nameGasRef, color = "red") %>%
                    dySeries(CalSet()$nameGasMod,label = CalSet()$nameGasMod, color = "blue") %>%
        ...
        if (input$UserMinsAvg == input$UserMins) {
            ...
            if(!is.null(DF$General)) Init.DF.aggregated <- data.frame(timeAverage(mydata   = DF$General, 
            ...
            #for (i in seq_along(list.gas.sensors())) Init.DF.aggregated[InComplete.Index[[i]],c(paste0("Out.",list.gas.reference2use()))] <- NA
        ...
        pointsExtrap <- reactive( {
            ...
            PointsExtrap <- DF$General[DF$General$date >= DateIN & DF$General$date <= DateEND,c("latitude","longitude", "Ref.Long", "Ref.Lat" )]
            
            # coordinates of the REFERENCE Station
            Ref.coord <- CalSet()$Coord.Ref
            
            # coordinates of the REFERENCE Station (Lon and Lat)
            if (any(!is.na(PointsExtrap$Ref.Long)) && any(!is.na(PointsExtrap$Ref.Lat))) {
                Ref.coord_LON <- mean(PointsExtrap$Ref.Long, na.rm = T)
                Ref.coord_LAT <- mean(PointsExtrap$Ref.Lat , na.rm = T)
            } else if (!is.null(Ref.coord) && grepl(pattern = " ", x = Ref.coord)) { 
                Ref.coord_LON <- as.numeric(unlist(strsplit(x = Ref.coord, split = " "))[1])
                Ref.coord_LAT <- as.numeric(unlist(strsplit(x = Ref.coord, split = " "))[2])
            } else {
                Ref.coord_LON <- NULL
        ...
        output$ts_Extra_dygraphs <- renderDygraph({
            ...
            # progress <- shiny::Progress$new()
            # progress$set(message = "[shiny, ts_extra_dygraphs()] INFO, plotting time series of Calibration data", value = 0.5)
            # # Make sure it closes when we exit this reactive, even if there's an error
            # on.exit(progress$close())
            ...
            cat("\n")
            cat("-----------------------------------------------------------------------------------\n")
            cat("[shiny, ts_extra_dygraphs()] INFO, plotting time series of Calibration data\n")
            ...
            if (all(is.na(DF$General[,c(CalSet()$nameGasRef,CalSet()$nameGasVolt)]),na.rm = TRUE)) {
                ...
                # Sensor relationships with other variables
                cat(paste0("[shiny, ts_extra_dygraphs()] INFO, Plot calibrate sensor data and reference data for Sensor ", input$Sensors, " in order to check relationships with other variables\n"))
                Pattern  <- rbind(c("Out.", ""),c("Ref.", "Reference "),c("ppm",""),c("ppb",""),c("_modelled",""), c("_",""))
                if (nrow(Pattern) > 0) {
                    Name.pol <- c(gsub(pattern = Pattern[1,1], replacement = Pattern[1,2], x = CalSet()$nameGasRef), gsub(pattern = Pattern[1,1], replacement = Pattern[1,2], x = CalSet()$nameGasMod))
                } 
                ...
                mydata = subset(DF.aggregated$Avg, date >= DateIN & date <= DateEND)
                mydata <- mydata %>%
                    select(date,
                           CalSet()$nameGasRef,
                           CalSet()$nameGasMod)
                ...
                ts_extra <- cbind(time_series_sensor[[CalSet()$nameGasRef]], 
                                time_series_sensor[[CalSet()$nameGasMod]])
